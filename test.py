...
def inference(model):
    ...
    # out
    print(str(epoch) + "\tth model Test: loss: {:0.4f}  dice={:0.4f}  miou={:0.4f}  precision={:0.4f}  recall={:0.4f}"
                .format(loss_record_test.show(), mean_dice, mean_iou, mean_precision, mean_recall))
    # out txt
    f = f"snapshots/{modelname}/{datasetname}_test.txt"
    with open(f, "a") as file1:  # ”w"代表着每次运行都覆盖内容
        file1.write("\n" + str(epoch) + "\tth model Test: loss: {:0.4f}  dice={:0.4f}  miou={:0.4f}  precision={:0.4f}  recall={:0.4f}"
                .format(loss_record_test.show(), mean_dice, mean_iou, mean_precision, mean_recall))
...
if __name__ == '__main__':
    ...
    # modify allocation
    datasetname = 'CVC-300'
    datasetpath = f'./datakvasir82/TestDataset/' # Kvasir CVC-ClinicDB CVC-ColonDB ETIS-LaribPolypDB CVC-300
    backbone = 'b2' # b2 b3
