    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, pack in enumerate(train_loader, start=1):
            ...
            for rate in size_rates: 
                ...
                # ---- forward ----
                # map3, map2, map1 = model(images)
                # loss = structure_loss(map1, gts) + structure_loss(map2, gts) + structure_loss(map3, gts)
                map3 = model(images)
                loss = structure_loss(map3, gts)
                # with torch.autograd.set_detect_anomaly(True):
                #loss = nn.functional.binary_cross_entropy(map1, gts)
                # ---- metrics ----
                dice_score = dice_m(map3, gts)
                iou_score = iou_m(map3, gts)
                # ---- backward ----
                loss.backward()
...
if __name__ == '__main__':
    ...
    # mod decoder ssformer F3Net
    # from mmseg.models.segmentors import colonformer_ssformer_mod_0322 as UNet
    # mod decoder ssformer 0323
    from mmseg.models.segmentors import colonformer_ssformer_mod_0323 as UNet
