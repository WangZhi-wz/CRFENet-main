...
from torch.nn import Conv2d, UpsamplingBilinear2d, init, Parameter, BatchNorm2d
...
class FullyAttentionalBlock(nn.Module):
    def __init__(self, plane, norm_layer=BatchNorm2d):
...
class Decoder(nn.Module):
    ...
    def __init__(self, dims, dim, class_num=2):
        ...
        self.edge_conv_cat = BasicConv2d(128 * 3, 128, kernel_size=3, padding=1)
        self.edge_linear = nn.Conv2d(192, 1, kernel_size=3, padding=1)

        self.sideout3 = SideoutBlock(512, 1)
        self.sideout2 = SideoutBlock(320, 1)
        self.sideout1 = SideoutBlock(128, 1)

        self.fa = FullyAttentionalBlock(192)
    ...
    def forward(self, inputs):
        ...
        c_all = torch.cat((c2_edge, c3_edge, c4_edge), dim=1)
        c_all = self.fa(c_all)

        # edge_cat = self.edge_conv_cat(c_all) # 1 64 88 88
        edge_map = self.edge_linear(c_all) # 1 1 88 88
...
class colonformer_ssformer_0403_AFF_BG_FA(nn.Module):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(colonformer_ssformer_0403_AFF_BG_FA, self).__init__()
