...
class Decoder(nn.Module):
    ...
    def __init__(self, dims, dim, class_num=2):
        ...
        self.RME = RF2B(192, 192)

        # BasicConv2d(64, 64, kernel_size=3, padding=1)
        self.edge_conv1 = nn.Sequential(BasicConv2d(64, 64, kernel_size=3, stride=2, padding=1), BasicConv2d(64, 64, kernel_size=3, stride=2, padding=1), nn.MaxPool2d(kernel_size=2, stride=2))
        self.edge_conv2 = nn.Sequential(BasicConv2d(128, 64, kernel_size=3, padding=1), nn.MaxPool2d(kernel_size=2, stride=2))
    ...
    def forward(self, inputs):
        ...
        c4_edge = self.upsample2(c4_edge) # 1 64 88 88

        # edge_cat = self.RME(torch.cat((c1_edge, c2_edge, c3_edge, c4_edge), dim=1)) # 1 64 88 88
        # edge_cat = self.RME(torch.cat((c2_edge, c3_edge, c4_edge), dim=1)) # 1 64 88 88
        edge_cat = self.edge_conv_cat(torch.cat((c2_edge, c3_edge, c4_edge), dim=1))  # 1 64 88 88

        edge_map = self.edge_linear(edge_cat) # 1 1 88 88
        edge_map1 = F.interpolate(edge_map, size=c1.size()[2:], mode='bilinear')
        edge_map2 = F.interpolate(edge_map, size=c2.size()[2:], mode='bilinear')
        edge_map3 = F.interpolate(edge_map, size=c3.size()[2:], mode='bilinear')
...
class colonformer_ssformer_0403_AFF_BGRes2Net(nn.Module):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(colonformer_ssformer_0403_AFF_BGRes2Net, self).__init__()
