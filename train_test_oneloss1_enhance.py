...
if __name__ == '__main__':
    ...
    # dataset   kvasir_SEG CVC_ClinicDB piccolo esophagus
    parser.add_argument('--dataset', type=str,
                        default='kvasir_SEG', help='dataset')
    parser.add_argument('--train_path', type=str,
                        default='./datakvasir82/TrainDataset', help='path to train dataset')
    parser.add_argument('--test_path', type=str,
                        default='./datakvasir82/TestDataset', help='path to test dataset')
    ...
    # load data
    train_data = getattr(datasets, args.dataset)('./datakvasir82/', 'TrainDataset', mode='train')
    train_dataloader = DataLoader(train_data, args.batchsize, shuffle=True, num_workers=0)

    valid_data = getattr(datasets, args.dataset)('./datakvasir82/', 'TestDataset', mode='test')
    valid_dataloader = DataLoader(valid_data, batch_size=1, shuffle=False, num_workers=0)
    val_total_batch = int(len(valid_data) / 1)

    # test_data = getattr(datasets, args.dataset)(args.test_path, mode="test")
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        train(train_dataloader, model, optimizer, epoch, lr_scheduler, args)
