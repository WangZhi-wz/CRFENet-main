    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, pack in enumerate(train_loader, start=1):
            ...
            for rate in size_rates: 
                ...
                # ---- forward ----
                map4, map2, map1 = model(images)
                loss = structure_loss(map4, gts) + structure_loss(map2, gts) + structure_loss(map1, gts)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str,
                        default='kvasir82', help='path to dataset')
    ...
    from mmseg.models.segmentors.DS_TransUNet import UNett as UNet

    model = UNet(128, n_class=1).cuda()
    ...
    # 将模型写入tensorboard
    init_img = torch.zeros((1, 3, 256, 256)).cuda()
