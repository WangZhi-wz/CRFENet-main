...
def inference(model):
    ...
    # out
    print("\n" + str(ii) + "\tth model Result: loss: {:0.4f}  dice={:0.4f}  miou={:0.4f}  precision={:0.4f}  recall={:0.4f}"
...
if __name__ == '__main__':
    ...
    # modify allocation
    datasetname = 'CVC-ColonDB'
    datasetpath = f'./dataall/TestDataset/CVC-ColonDB'
    ...
    print("Test on " + datasetname + "Dataset!!!")
    ii = 1
    for i in range(epoch):
        if weight != '':
            checkpoint = torch.load(f"../../hy-tmp/snapshots/{modelname}/model_save/{str(i + 1)}.pth")
