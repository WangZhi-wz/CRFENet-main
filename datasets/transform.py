...
# random_dilation_erosion   normalize
...
class Resize(object):
    ...
    def __call__(self, data):
        ...
        return {'image': cv2.resize(image, self.size), 'label': cv2.resize(label, self.size)}
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < self.p:
            return {'image': cv2.flip(image, 1), 'label': cv2.flip(label, 1)}
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < self.p:
            return {'image': cv2.flip(image, 0), 'label': cv2.flip(image, 0)}
...
class RandomRotation(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < 0.5:
            ...
            # print(angle)
            return {'image': cv2.rotate(image, int(angle)),
                    'label': cv2.rotate(label, int(angle))}
