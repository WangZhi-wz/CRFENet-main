...
class piccolo(Dataset):
    def __init__(self, root, data_dir, mode='train', transform=None):
        ...
        for data1 in datalist1:
            self.imglist.append(osp.join(data_path1 + '/images', data1))
            data2 = data1.split(".")[0] + "_Corrected.tif"
            # print(data2)
            self.gtlist.append(osp.join(data_path1 + '/masks', data2))
