...
class testtestDataset(torch.utils.data.Dataset):
...
def inference(model, args):
    ...
    X_test = glob('{}/test/images/*'.format(args.data_path))
    X_test.sort()
    y_test = glob('{}/test/masks/*'.format(args.data_path))
    y_test.sort()

    test_dataset = testtestDataset(X_test, y_test)
    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, pack in enumerate(train_loader, start=1):
            ...
            for rate in size_rates: 
                ...
                images = F.upsample(images, size=(192, 256), mode='bilinear', align_corners=True)
                gts = F.upsample(gts, size=(192, 256), mode='bilinear', align_corners=True)
                # ---- forward ----
                map1, map2, map4 = model(images)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init_trainsize', type=int,
                        default=(192, 256), help='training dataset size')
    parser.add_argument('--clip', type=float,
                        default=0.5, help='gradient clipping margin')
    parser.add_argument('--data_path', type=str,
                        default='ISIC811', help='path to dataset')
    ...
    train_img_paths = []
    train_mask_paths = []
    train_img_paths = glob('{}/train/images/*'.format(args.data_path))
    train_mask_paths = glob('{}/train/masks/*'.format(args.data_path))
    ...
    from mmseg.models.segmentors import TransFuse_L as UNet
    ...
    # ---- flops and params ----
    input = torch.randn(1, 3, 192, 256).to('cuda')
    ...
    # 将模型写入tensorboard
    init_img = torch.zeros((1, 3, 192, 256)).cuda()
