...
def inference(model):
    ...
    # 自己要放CAM的位置-----------------------------------------------------------------------------------------------
    layername = 'linear_pred'
    target_layers = [model.decode_head.linear_pred]  # model.decode_head.linear_pred  model.backbone.block4  out2
    # convc4c1  convc4c2  convc4c3  convc4c4

    # iaff3 iaff2 iaff1 dp4 dp3 dp2 dp1 block1 block2 block3 block4
...
if __name__ == '__main__':
    ...
    # modify allocation
    datasetname = 'COVID'
    datasetpath = f'./COVID/test' # Kvasir CVC-ClinicDB CVC-ColonDB ETIS-LaribPolypDB CVC-300
    # kvasir82  CVC_ClinicDB811  CVC-ColonDB811  ETIS-LaribPolypDB82  ISIC2017  ISIC811  bowl  GLas  COVID
    # modelpath = r"C:\Users\15059\Desktop\Paper2model\0508_DP_AFF\16_ClinicDB.pth"
    backbone = 'b3' # b2 b3
    ...
    from mmseg.models.segmentors import colonformer_ssformer_add as UNet



    model = UNet(backbone=dict(
                    type='mit_{}'.format(backbone),  # pvt_v2_b2
                    # type='pvt_medium',
                ...
                pretrained='pretrained/mit_{}.pth'.format(backbone)
                # pretrained='pretrained/pvt_medium_iter_40000.pth'
                # pretrained='pretrained/pvt_v2_{}.pth'.format(args.backbone)
    ).cuda()
