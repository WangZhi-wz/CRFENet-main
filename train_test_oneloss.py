...
def inference(model, args):
    ...
    # out
    print("Test Result: loss: {:0.4f}, dice={:0.4f}, miou={:0.4f}, precision={:0.4f}, recall={:0.4f} \n"
    ...
    with open(f, "a") as filenametest:  # ”w"代表着每次运行都覆盖内容
        filenametest.write("\nTest Result: loss: {:0.4f}, dice={:0.4f}, miou={:0.4f}, precision={:0.4f}, recall={:0.4f} \n"
                .format(loss_record_test.show(), mean_dice, mean_iou, mean_precision, mean_recall))
    filenametest.close()
    ...
    with torch.autograd.set_detect_anomaly(True):
        for i, pack in enumerate(train_loader, start=1):
            ...
            for rate in size_rates: 
                ...
                # ---- forward ----
                map = model(images)
                # map, map2, map3 = model(images)
                map = F.upsample(map, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                ...
                # loss = structure_loss(map3, gts) + structure_loss(map2, gts) + structure_loss(map, gts)
                loss = structure_loss(map, gts)
                # with torch.autograd.set_detect_anomaly(True):
                #loss = nn.functional.binary_cross_entropy(map1, gts)
                # ---- metrics ----
                dice_score = dice_m(map, gts)
                iou_score = iou_m(map, gts)
                # ---- backward ----
                loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', type=str,
                        default='b2', help='backbone version')
    ...
    parser.add_argument('--train_path', type=str,
                        default='./datatest/TrainDataset', help='path to train dataset')
    ...
    parser.add_argument('--test_path', type=str,
                        default='./datatest/TestDataset', help='path to dataset') # Kvasir
    ...
    # van + msrdm
    # from mmseg.models.segmentors import colonformer_ssformer_vanmsrdm as UNet
    # U-Net Decoder
    ...
    model = UNet(backbone=dict(
                ...
                pretrained='../pycharm_project_94/pretrained/mit_{}.pth'.format(args.backbone)).cuda()
    ...
    print(f"batchsize:{args.batchsize}  train_path:{args.train_path}  test_path:{args.test_path}")
    f = f"{save_path}/result/train.txt"
    with open(f, "a") as filenametest:  # ”w"代表着每次运行都覆盖内容
        filenametest.write(f"\n\n\n\n\nstart training: {args.train_save}  macs: {macs / 1000000000}, params: {params / 1000000}, time:{x.strftime('%Y-%m-%d %H:%M:%S')}"
                           f"\nbatchsize:{args.batchsize}  train_path:{args.train_path}  test_path:{args.test_path}")
    filenametest.close()
