...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            # augmented = self.transform(image=image, mask=mask)
            image, mask = self.transform(data)
            # image = self.transform(['images'])
            # mask = self.transform(['masks'])
        else:
            image = cv2.resize(image, (352, 352))
            mask = cv2.resize(mask, (352, 352))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='./Kvasir91/train', help='path to train dataset')
    parser.add_argument('--test_path', type=str,
                        default='./Kvasir91/test', help='path to dataset')
    ...
    train_dataset = TrainDataset(train_img_paths, train_mask_paths)
